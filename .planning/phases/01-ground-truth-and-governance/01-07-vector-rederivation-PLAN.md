...
must_haves:
  truths:
    ...
    - "After any convention re-wording, the convention document still satisfies the documented-conventions test: every key has exactly one recorded decision, the document's heading order still equals the type's key order, and every decision still carries its `Default origin:` line in one of the four recognised forms — a rewording that changes a default must restate where the new default came from, and may not quietly drop the attribution"
